...
- `Sources/TokenUsage`: Swift 6 menu bar app (usage/credits probes, icon renderer, settings). Keep changes small and reuse existing helpers.
- `Tests/TokenUsageTests`: XCTest coverage for usage parsing, status probes, icon patterns; mirror new logic with focused tests.
...
## Build, Test, Run
- Dev loop: `./Scripts/compile_and_run.sh` kills old instances, runs `swift build` + `swift test`, packages, relaunches `TokenUsage.app`, and confirms it stays running.
- Quick build/test: `swift build` (debug) or `swift build -c release`; `swift test` for the full XCTest suite.
- Package locally: `./Scripts/package_app.sh` to refresh `TokenUsage.app`, then restart with `pkill -x TokenUsage || pkill -f TokenUsage.app || true; cd /Users/steipete/Projects/tokenusage && open -n /Users/steipete/Projects/tokenusage/TokenUsage.app`.
...
## Testing Guidelines
- Add/extend XCTest cases under `Tests/TokenUsageTests/*Tests.swift` (`FeatureNameTests` with `test_caseDescription` methods).
...
- To guarantee the right bundle is running after a rebuild, use: `pkill -x TokenUsage || pkill -f TokenUsage.app || true; cd /Users/steipete/Projects/tokenusage && open -n /Users/steipete/Projects/tokenusage/TokenUsage.app`.
