    ...
    func `fetch usage sets credits timeout and client headers`() async throws {
        ...
        OpenRouterStubURLProtocol.handler = { request in
            ...
            case "/api/v1/credits":
                ...
                #expect(request.value(forHTTPHeaderField: "HTTP-Referer") == "https://tokenusage.example")
                #expect(request.value(forHTTPHeaderField: "X-Title") == "TokenUsage QA")
        ...
        let usage = try await OpenRouterUsageFetcher.fetchUsage(
            ...
            environment: [
                ...
                "OPENROUTER_HTTP_REFERER": " https://tokenusage.example ",
                "OPENROUTER_X_TITLE": "TokenUsage QA",
            ])
