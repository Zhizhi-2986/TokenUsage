...
struct CLIProviderSelectionTests {
    ...
    func `help includes gemini and all`() {
        let usage = TokenUsageCLI.usageHelp(version: "0.0.0")
        let root = TokenUsageCLI.rootHelp(version: "0.0.0")
        ...
        #expect(usage.contains("tokenusage usage --provider gemini"))
        #expect(usage.contains("tokenusage usage --format json --provider all --pretty"))
        #expect(root.contains("tokenusage --provider gemini"))
    }
    ...
    func `provider selection respects override`() {
        let selection = TokenUsageCLI.providerSelection(rawOverride: "gemini", enabled: [.codex, .claude])
        #expect(selection.asList == [.gemini])
    }

    @Test
    func `provider selection uses enabled providers when three or more are enabled`() {
        let selection = TokenUsageCLI.providerSelection(
            rawOverride: nil,
    ...
    func `provider selection does not expand three enabled providers to all providers`() {
        ...
        let selection = TokenUsageCLI.providerSelection(rawOverride: nil, enabled: enabled)
    ...
    func `provider selection accepts kiro alias`() {
        let selection = TokenUsageCLI.providerSelection(rawOverride: "kiro-cli", enabled: [.codex])
