...
@Suite(.serialized)
struct TokenUsageConfigMigratorTests {
    @Test
    func `legacy secret migration completion flag skips repeated scans`() throws {
        let suite = "TokenUsageConfigMigratorTests-skip-\(UUID().uuidString)"
        ...
        _ = TokenUsageConfigMigrator.loadOrMigrate(configStore: configStore, userDefaults: defaults, stores: stores)
    ...
    func `legacy migration completion waits for successful cleanup`() throws {
        let suite = "TokenUsageConfigMigratorTests-cleanup-failure-\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suite))
        defaults.removePersistentDomain(forName: suite)
        defer { defaults.removePersistentDomain(forName: suite) }
    ...
    private static let legacyMigrationCompletedKey = "tokenusage.legacySecretsMigrationCompleted"

    private static func legacyStores(
        secrets: CountingLegacySecretStore,
        accountStore: CountingTokenAccountStore) -> TokenUsageConfigMigrator.LegacyStores
    {
        TokenUsageConfigMigrator.LegacyStores(
...
private final class CountingTokenAccountStore: ProviderTokenAccountStoring, @unchecked Sendable {
    ...
    func ensureFileExists() throws -> URL {
        FileManager.default.temporaryDirectory.appendingPathComponent("tokenusage-empty-accounts.json")
    }
}
