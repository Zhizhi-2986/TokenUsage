...
struct CLIOutputTests {
    ...
    func `cli error payload is JSON array`() throws {
        let payload = TokenUsageCLI.makeCLIErrorPayload(
            message: "Nope",
            code: .failure,
            kind: .args,
            pretty: false)
    ...
    func `exit omits generic error when command already emitted payload`() {
        #expect(!TokenUsageCLI.shouldPrintExitError(code: .success, message: nil))
        #expect(!TokenUsageCLI.shouldPrintExitError(code: .failure, message: nil))
        #expect(TokenUsageCLI.shouldPrintExitError(code: .failure, message: "Nope"))
    }
}
