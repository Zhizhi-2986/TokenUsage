...
struct TokenUsageWidgetProviderTests {
    @Test
    ...
    func `supported providers fall back to codex when snapshot is empty`() {
        ...
        #expect(TokenUsageSwitcherTimelineProvider.supportedProviders(from: snapshot) == [.codex])
    }
    ...
    func `supported providers keep alibaba when it is the only enabled provider`() {
        ...
        #expect(TokenUsageSwitcherTimelineProvider.supportedProviders(from: snapshot) == [.alibaba])
    }
