...
struct CLIArgumentParsingTests {
    ...
    func `json output flag enables json logs`() throws {
        ...
        #expect(parsed.flags.contains("jsonOutput"))
        #expect(!parsed.flags.contains("jsonShortcut"))
        #expect(TokenUsageCLI._decodeFormatForTesting(from: parsed) == .text)
    }
    ...
    func `resolved log level defaults to error`() {
        #expect(TokenUsageCLI.resolvedLogLevel(verbose: false, rawLevel: nil) == .error)
        #expect(TokenUsageCLI.resolvedLogLevel(verbose: true, rawLevel: nil) == .debug)
        #expect(TokenUsageCLI.resolvedLogLevel(verbose: false, rawLevel: "info") == .info)
    }
