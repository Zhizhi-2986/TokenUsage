...
- Build script: make TokenUsage-owned ad-hoc keychain cleanup opt-in with `--clear-adhoc-keychain`, and extend the explicit reset path to clear both `com.steipete.TokenUsage` and `com.steipete.tokenusage.cache`. Thanks @magnaprog!
...
### Claude OAuth & Keychain
- Claude OAuth creds are cached in TokenUsage Keychain to reduce repeated prompts.
...
- CLI/App: move provider secrets + ordering to `~/.tokenusage/config.json` (no Keychain persistence).
...
- CLI: add `tokenusage cost` to print local cost usage (text/JSON) for Codex + Claude.
...
- macOS: TokenUsage now supports Intel Macs (x86_64 builds + Sonoma fallbacks). Thanks @epoyraz!
...
- Homebrew: cask now installs the `tokenusage` CLI symlink. Thanks @dalisoft!
...
- Widgets: add a new “TokenUsage Switcher” widget that lets you switch providers and remember the selection.
...
- CI: build/test Linux `TokenUsageCLI` (x86_64 + aarch64) and publish release assets as `TokenUsageCLI-<tag>-linux-<arch>.tar.gz` (+ `.sha256`).
...
## 0.8.0 — 2025-12-17
- TokenUsage is now available via Homebrew: `brew install --cask steipete/tap/tokenusage` (updates via `brew upgrade --cask steipete/tap/tokenusage`).
...
## 0.6.0 — 2025-12-04
- New bundled CLI (`tokenusage`) with single `usage` command, `--format text|json`, `--status`, and fast `-h/-V`.
- CLI output now shows consistent headers (`Codex 0.x.y (codex-cli)`, `Claude Code <ver> (claude)`) and JSON includes `source` + `status`.
- Advanced prefs install button symlinks `tokenusage` into /usr/local/bin and /opt/homebrew/bin; docs refreshed.
...
- Debug replay toggle (`defaults write com.steipete.tokenusage debugMenuEnabled -bool YES`) to view every pattern.
...
- Notarized release build "TokenUsage-0.1.0.zip" remains current artifact; app version 0.1.1.

## 0.1.0 — 2025-11-16
- Initial TokenUsage release: macOS 15+ menu bar app, no Dock icon.
