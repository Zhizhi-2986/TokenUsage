...
struct TokenUsageWidgetEntry: TimelineEntry {
    let date: Date
    let provider: UsageProvider
    let snapshot: WidgetSnapshot
}

struct TokenUsageCompactEntry: TimelineEntry {
    let date: Date
    let provider: UsageProvider
...
struct TokenUsageSwitcherEntry: TimelineEntry {
    let date: Date
    let provider: UsageProvider
...
struct TokenUsageTimelineProvider: AppIntentTimelineProvider {
    func placeholder(in context: Context) -> TokenUsageWidgetEntry {
        TokenUsageWidgetEntry(
            date: Date(),
            provider: .codex,
            snapshot: WidgetPreviewData.snapshot())
    }

    func snapshot(for configuration: ProviderSelectionIntent, in context: Context) async -> TokenUsageWidgetEntry {
        let provider = configuration.provider.provider
        return TokenUsageWidgetEntry(
            date: Date(),
            provider: provider,
    ...
    func timeline(
        ...
        in context: Context) async -> Timeline<TokenUsageWidgetEntry>
    {
        let provider = configuration.provider.provider
        let snapshot = WidgetSnapshotStore.load() ?? WidgetPreviewData.emptySnapshot()
        let entry = TokenUsageWidgetEntry(date: Date(), provider: provider, snapshot: snapshot)
        let refresh = Date().addingTimeInterval(30 * 60)
        return Timeline(entries: [entry], policy: .after(refresh))
    }
}

struct TokenUsageSwitcherTimelineProvider: TimelineProvider {
    func placeholder(in context: Context) -> TokenUsageSwitcherEntry {
        let snapshot = WidgetPreviewData.snapshot()
        let providers = self.availableProviders(from: snapshot)
        return TokenUsageSwitcherEntry(
            date: Date(),
    ...
    func getSnapshot(in context: Context, completion: @escaping (TokenUsageSwitcherEntry) -> Void) {
        completion(self.makeEntry())
    }

    func getTimeline(in context: Context, completion: @escaping (Timeline<TokenUsageSwitcherEntry>) -> Void) {
        let entry = self.makeEntry()
        let refresh = Date().addingTimeInterval(30 * 60)
        completion(Timeline(entries: [entry], policy: .after(refresh)))
    }

    private func makeEntry() -> TokenUsageSwitcherEntry {
        let snapshot = WidgetSnapshotStore.load() ?? WidgetPreviewData.emptySnapshot()
        let providers = self.availableProviders(from: snapshot)
...
struct TokenUsageCompactTimelineProvider: AppIntentTimelineProvider {
    func placeholder(in context: Context) -> TokenUsageCompactEntry {
        TokenUsageCompactEntry(
            date: Date(),
            provider: .codex,
            metric: .credits,
            snapshot: WidgetPreviewData.snapshot())
    ...
    func snapshot(for configuration: CompactMetricSelectionIntent, in context: Context) async -> TokenUsageCompactEntry {
        let provider = configuration.provider.provider
        let metric = configuration.metric
        return TokenUsageCompactEntry(
            date: Date(),
            provider: provider,
            metric: metric,
            snapshot: WidgetSnapshotStore.load() ?? WidgetPreviewData.snapshot())
    ...
    func timeline(
        ...
        in context: Context) async -> Timeline<TokenUsageCompactEntry>
    {
        let provider = configuration.provider.provider
        let metric = configuration.metric
        let snapshot = WidgetSnapshotStore.load() ?? WidgetPreviewData.emptySnapshot()
        let entry = TokenUsageCompactEntry(
            date: Date(),
            provider: provider,
            metric: metric,
            snapshot: snapshot)
