...
extension TokenUsageCLI {
    static func decodeProvider(from values: ParsedValues, config: TokenUsageConfig) -> ProviderSelection {
    ...
    static func resetTimeDisplayStyleFromDefaults() -> ResetTimeDisplayStyle {
        let domains = [
            "com.steipete.tokenusage",
            "com.steipete.tokenusage.debug",
        ]
    ...
    static func loadConfig(output: CLIOutputPreferences) -> TokenUsageConfig {
        let store = TokenUsageConfigStore()
        do {
            if let existing = try store.load() {
                return existing
            }
            return TokenUsageConfig.makeDefault()
        } catch {
