...
enum TokenUsageCLI {
    ...
    private static func bootstrapLogging(values: ParsedValues) {
        ...
        TokenUsageLog.bootstrapIfNeeded(.init(destination: .stderr, level: level, json: isJSON))
    }

    static func resolvedLogLevel(verbose: Bool, rawLevel: String?) -> TokenUsageLog.Level {
        TokenUsageLog.parseLevel(rawLevel) ?? (verbose ? .debug : .error)
    }
