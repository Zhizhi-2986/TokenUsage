...
final class OpenAIDashboardWebViewCache {
    ...
    fileprivate static let log = TokenUsageLog.logger(LogCategories.openAIWebview)
    ...
    (() => {
      try {
        delete window.__tokenusageDidScrollToCredits;
        delete window.__tokenusageUsageBreakdownJSON;
        delete window.__tokenusageUsageBreakdownDebug;
        return true;
      } catch {
        return false;
      }
