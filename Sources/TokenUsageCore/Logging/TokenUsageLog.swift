...
public enum TokenUsageLog {
    ...
    public static func logger(_ category: String) -> TokenUsageLogger {
        let logger = Logger(label: "com.steipete.tokenusage.\(category)")
        return TokenUsageLogger { level, message, metadata in
...
public struct TokenUsageLogger: Sendable {
    private let logFn: @Sendable (TokenUsageLog.Level, String, [String: String]?) -> Void

    fileprivate init(_ logFn: @escaping @Sendable (TokenUsageLog.Level, String, [String: String]?) -> Void) {
        self.logFn = logFn
    }
