...
public struct TTYCommandRunner {
    private static let log = TokenUsageLog.logger(LogCategories.ttyRunner)
    ...
    static func locateBundledHelper(_ name: String) -> String? {
        ...
        if let override = ProcessInfo.processInfo.environment["TOKENUSAGE_HELPER_\(name.uppercased())"],
        ...
        onURLDetected: (@Sendable () -> Void)? = nil) throws -> Result
    {
        ...
        if resolvedURL.lastPathComponent == "claude",
           let watchdog = Self.locateBundledHelper("TokenUsageClaudeWatchdog")
        {
