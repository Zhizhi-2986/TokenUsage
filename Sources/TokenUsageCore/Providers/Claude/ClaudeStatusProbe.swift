...
public struct ClaudeStatusProbe: Sendable {
    ...
    private static let log = TokenUsageLog.logger(LogCategories.claudeProbe)
    #if DEBUG
    ...
    private static func extractUsageError(text: String) -> String? {
        ...
        if lower.contains("do you trust the files in this folder?"), !lower.contains("current session") {
            ...
            if let folderHint {
                return """
                Claude CLI is waiting for a folder trust prompt (\(folderHint)). TokenUsage tries to auto-accept this, \
                but if it keeps appearing run: `cd "\(folderHint)" && claude` and choose “Yes, proceed”, then retry.
                """
            }
            return """
            Claude CLI is waiting for a folder trust prompt. TokenUsage tries to auto-accept this, but if it keeps \
