...
actor ClaudeCLISession {
    ...
    private static let log = TokenUsageLog.logger(LogCategories.claudeCLI)
    ...
    private func ensureStarted(binary: String) throws {
        ...
        let disableWatchdog = ProcessInfo.processInfo.environment["TOKENUSAGE_DISABLE_CLAUDE_WATCHDOG"] == "1"
        if !disableWatchdog,
           resolvedURL.lastPathComponent == "claude",
           let watchdog = TTYCommandRunner.locateBundledHelper("TokenUsageClaudeWatchdog")
        {
