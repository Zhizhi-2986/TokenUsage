...
public enum ClaudeOAuthCredentialsStore {
    ...
    public static let environmentTokenKey = "TOKENUSAGE_CLAUDE_OAUTH_TOKEN"
    public static let environmentScopesKey = "TOKENUSAGE_CLAUDE_OAUTH_SCOPES"
    ...
    public static let environmentClientIDKey = "TOKENUSAGE_CLAUDE_OAUTH_CLIENT_ID"
    ...
    static let log = TokenUsageLog.logger(LogCategories.claudeUsage)
        ...
        func shouldAttemptFreshnessSyncFromClaudeKeychain(cached: ClaudeOAuthCredentialRecord) -> Bool {
            ...
            case .onlyOnUserAction:
                if ProviderInteractionContext.current != .userInitiated {
                    if ProcessInfo.processInfo.environment["TOKENUSAGE_DEBUG_CLAUDE_OAUTH_FLOW"] == "1" {
                        ClaudeOAuthCredentialsStore.log.debug(
    ...
    /// - TokenUsage-owned credentials refresh directly via token endpoint.
    ...
    /// Save refreshed credentials to TokenUsage's keychain cache
    private static func saveRefreshedCredentialsToCache(_ credentials: ClaudeOAuthCredentials) {
        ...
        self.saveToCacheKeychain(jsonData, owner: .tokenusage)
        self.log.debug("Saved refreshed credentials to TokenUsage keychain cache")
    }
    ...
    /// Check if TokenUsage has cached credentials (in memory or keychain cache)
...
extension ClaudeOAuthCredentialsStore {
    ...
    /// TokenUsage's caches. This is used to avoid triggering a second OS keychain dialog during the OAuth retry.
    @discardableResult
    ...
    /// Updates TokenUsage's keychain cache with the new credentials.
