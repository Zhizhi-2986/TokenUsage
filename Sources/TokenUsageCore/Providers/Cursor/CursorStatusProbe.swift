...
public enum CursorStatusProbeError: LocalizedError, Sendable {
    ...
    public var errorDescription: String? {
        switch self {
        case .notLoggedIn:
            "Not logged in to Cursor. Please log in via the TokenUsage menu."
        ...
        case .noSessionCookie:
            "No Cursor session found. Please log in to cursor.com in \(cursorCookieImportOrder.loginHint). "
                + "If you use Safari, grant TokenUsage Full Disk Access in System Settings ▸ Privacy & Security. "
                + "You can also sign in to Cursor from the TokenUsage menu (Add / switch account)."
        }
    }
}
...
public actor CursorSessionStore {
    ...
    private init() {
        ...
        let dir = appSupport.appendingPathComponent("TokenUsage", isDirectory: true)
