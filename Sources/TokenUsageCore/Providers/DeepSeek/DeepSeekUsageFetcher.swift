...
// MARK: - Monthly expense tracking

private enum DeepSeekMonthlyTracker {
    ...
    static func monthlyExpenses(for currentBalance: Double) -> Double {
        let key = "DeepSeekFirstBalance_\(currentMonthKey)"
        let defaults = UserDefaults.standard
        let firstBalance = defaults.double(forKey: key)
        if firstBalance == 0 {
            defaults.set(currentBalance, forKey: key)
            return 0
        }
        let expenses = firstBalance - currentBalance
        if expenses < 0 {
            // Topped up this month, reset baseline
            defaults.set(currentBalance, forKey: key)
            return 0
        }
        return expenses
    ...
    static func resetForTesting() {
        UserDefaults.standard.removeObject(forKey: "DeepSeekFirstBalance_\(currentMonthKey)")
    }
    #endif
}
...
public struct DeepSeekUsageSnapshot: Sendable {
    ...
    public func toUsageSnapshot() -> UsageSnapshot {
        ...
        if self.totalBalance <= 0 {
            ...
            let expenses = String(format: "\(symbol)%.2f", self.monthlyExpenses)
            balanceDetail = "\(total) (Paid: \(paid) / Monthly expenses: \(expenses))"
            usedPercent = 0
        }
...
public struct DeepSeekUsageFetcher: Sendable {
    private static let log = TokenUsageLog.logger(LogCategories.deepSeekUsage)
    ...
    private static func parseSnapshot(data: Data) throws -> DeepSeekUsageSnapshot {
        ...
        let expenses = DeepSeekMonthlyTracker.monthlyExpenses(for: total)
