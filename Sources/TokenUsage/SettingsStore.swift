...
final class SettingsStore {
    ...
    @ObservationIgnored let configStore: TokenUsageConfigStore
    @ObservationIgnored var config: TokenUsageConfig
    ...
    init(
        ...
        configStore: TokenUsageConfigStore = TokenUsageConfigStore(),
        ...
        tokenAccountStore: any ProviderTokenAccountStoring = FileTokenAccountStore())
    {
        ...
        if !Self.isRunningTests {
            TokenUsageLog.logger(LogCategories.settings).info(
                "App group resolved",
                metadata: [
                    "groupID": appGroupID,
        ...
        let legacyStores = TokenUsageConfigMigrator.LegacyStores(
        ...
        let config = TokenUsageConfigMigrator.loadOrMigrate(
        ...
        TokenUsageLog.setFileLoggingEnabled(self.debugFileLoggingEnabled)
...
extension SettingsStore {
    ...
    private static func loadDefaultsState(userDefaults: UserDefaults) -> SettingsDefaultsState {
        ...
        let debugLogLevelRaw = userDefaults.string(forKey: "debugLogLevel") ?? TokenUsageLog.Level.verbose.rawValue
...
extension SettingsStore {
    var configSnapshot: TokenUsageConfig {
        _ = self.configRevision
        return self.config
    }

    func updateProviderState(config: TokenUsageConfig) {
    ...
    func setProviderEnabled(provider: UsageProvider, metadata _: ProviderMetadata, enabled: Bool) {
        TokenUsageLog.logger(LogCategories.settings).debug(
