...
final class UsageStore {
    ...
    @ObservationIgnored private let sessionQuotaLogger = TokenUsageLog.logger(LogCategories.sessionQuota)
    @ObservationIgnored let openAIWebLogger = TokenUsageLog.logger(LogCategories.openAIWeb)
    @ObservationIgnored private let tokenCostLogger = TokenUsageLog.logger(LogCategories.tokenCost)
    @ObservationIgnored let augmentLogger = TokenUsageLog.logger(LogCategories.augment)
    @ObservationIgnored let providerLogger = TokenUsageLog.logger(LogCategories.providers)
...
extension UsageStore {
    func debugDumpClaude() async {
        ...
        let url = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("tokenusage-claude-probe.txt")
    ...
    func dumpLog(toFileFor provider: UsageProvider) async -> URL? {
        ...
        let filename = "tokenusage-\(provider.rawValue)-probe.txt"
        ...
        fallbackCookieHeader: String) async -> ClaudeDebugLogConfiguration
    {
        await MainActor.run {
            ...
            return ClaudeDebugLogConfiguration(
                runtime: TokenUsageCore.ProviderRuntime.app,
