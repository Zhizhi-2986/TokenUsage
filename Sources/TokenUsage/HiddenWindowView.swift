...
struct HiddenWindowView: View {
    ...
    var body: some View {
        Color.clear
            ...
            .onReceive(NotificationCenter.default.publisher(for: .tokenusageOpenSettings)) { _ in
                Task { @MainActor in
                    self.openSettings()
                }
            }
            ...
            .onAppear {
                if let window = NSApp.windows.first(where: { $0.title == "TokenUsageLifecycleKeepalive" }) {
