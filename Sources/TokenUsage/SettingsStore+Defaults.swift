...
extension SettingsStore {
    ...
    var debugFileLoggingEnabled: Bool {
        ...
        set {
            ...
            TokenUsageLog.setFileLoggingEnabled(newValue)
        }
    }

    var debugLogLevel: TokenUsageLog.Level {
        get {
            let raw = self.defaultsState.debugLogLevelRaw
            return TokenUsageLog.parseLevel(raw) ?? .verbose
        }
        set {
            self.defaultsState.debugLogLevelRaw = newValue.rawValue
            self.userDefaults.set(newValue.rawValue, forKey: "debugLogLevel")
            TokenUsageLog.setLogLevel(newValue)
        }
    }
    ...
    var isVerboseLoggingEnabled: Bool {
        self.debugLogLevel.rank <= TokenUsageLog.Level.verbose.rank
    }
