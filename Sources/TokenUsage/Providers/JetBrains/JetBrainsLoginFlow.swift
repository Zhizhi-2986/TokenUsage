...
extension StatusItemController {
    func runJetBrainsLoginFlow() async {
        ...
        if detectedIDEs.isEmpty {
            let message = [
                "Install a JetBrains IDE with AI Assistant enabled, then refresh TokenUsage.",
            ...
            let message = hasQuotaFile
                ? "Detected: \(ideNames). Select your preferred IDE in Settings, then refresh TokenUsage."
                : "Detected: \(ideNames). Use AI Assistant once to generate quota data, then refresh TokenUsage."
            self.presentLoginAlert(
