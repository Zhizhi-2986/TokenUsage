        ...
        ScrollView(.vertical, showsIndicators: true) {
            VStack(alignment: .leading, spacing: 16) {
                ...
                SettingsSection(contentSpacing: 10) {
                    ...
                    Text("Symlink TokenUsageCLI to /usr/local/bin and /opt/homebrew/bin as tokenusage.")
                        .font(.footnote)
                        .foregroundStyle(.tertiary)
                }
...
extension AdvancedPane {
    private func installCLI() async {
        ...
        let helperURL = Bundle.main.bundleURL.appendingPathComponent("Contents/Helpers/TokenUsageCLI")
        let fm = FileManager.default
        guard fm.fileExists(atPath: helperURL.path) else {
            self.cliStatus = "TokenUsageCLI not found in app bundle."
            return
        }

        let destinations = [
            "/usr/local/bin/tokenusage",
            "/opt/homebrew/bin/tokenusage",
        ]
