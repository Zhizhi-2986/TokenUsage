...
struct TokenUsageConfigMigrator {
    struct LegacyStores {
    ...
    static func loadOrMigrate(
        configStore: TokenUsageConfigStore,
        userDefaults: UserDefaults,
        stores: LegacyStores) -> TokenUsageConfig
    {
        let log = TokenUsageLog.logger(LogCategories.configMigration)
        let existing = try? configStore.load()
        var config = (existing ?? TokenUsageConfig.makeDefault()).normalized()
    ...
    private static func applyProviderOrder(_ raw: [String], config: TokenUsageConfig) -> TokenUsageConfig {
    ...
    private static func applyProviderToggles(
        ...
        config: TokenUsageConfig) -> TokenUsageConfig
    {
        var updated = config
