...
struct TokenUsageApp: App {
    ...
    init() {
        ...
        let storedLevel = TokenUsageLog.parseLevel(UserDefaults.standard.string(forKey: "debugLogLevel")) ?? .verbose
        let level = TokenUsageLog.parseLevel(env["TOKENUSAGE_LOG_LEVEL"]) ?? storedLevel
        TokenUsageLog.bootstrapIfNeeded(.init(
            destination: .oslog(subsystem: "com.steipete.tokenusage"),
            level: level,
            json: false))
        ...
        TokenUsageLog.logger(LogCategories.app).info(
            "TokenUsage starting",
            metadata: [
                "version": version,
                "build": build,
        ...
        TokenUsageLog.setLogLevel(settings.debugLogLevel)
    ...
    var body: some Scene {
        ...
        WindowGroup("TokenUsageLifecycleKeepalive") {
            HiddenWindowView()
        }
...
private func makeUpdaterController() -> UpdaterProviding {
    ...
    if InstallOrigin.isHomebrewCask(appBundleURL: bundleURL) {
        return DisabledUpdaterController(
            unavailableReason: "Updates managed by Homebrew. Run: brew upgrade --cask steipete/tap/tokenusage")
    }
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private let confettiLogger = TokenUsageLog.logger(LogCategories.confetti)
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if !self.hasInstalledWeeklyLimitResetObserver {
            NotificationCenter.default.addObserver(
                ...
                name: .tokenusageWeeklyLimitReset,
                object: nil)
    ...
    private func ensureStatusController() {
        ...
        TokenUsageLog.logger(LogCategories.app)
