...
extension SettingsStore {
    private func updateConfig(reason: String, mutate: (inout TokenUsageConfig) -> Void) {
    ...
    func updateProviderTokenAccounts(_ accounts: [UsageProvider: ProviderTokenAccountData]) {
        let summary = accounts
            ...
            .joined(separator: ",")
        TokenUsageLog.logger(LogCategories.tokenAccounts).info(
    ...
    func reloadConfig(reason: String) {
        ...
        do {
            ...
            TokenUsageLog.logger(LogCategories.configStore).error("Failed to reload config: \(error)")
        }
    }

    func applyExternalConfig(_ config: TokenUsageConfig, reason: String) {
        guard !self.configLoading else { return }
    ...
    func schedulePersistConfig() {
        ...
        if Self.isRunningTests {
            do {
                ...
                TokenUsageLog.logger(LogCategories.configStore).error("Failed to persist config: \(error)")
            }
            return
        }
