    ...
    static func mapUserFacingError(_ error: Error) -> CodexSystemAccountPromotionUserFacingError {
        ...
        if let error = error as? CodexAccountPromotionError {
            ...
            case .targetManagedAccountNotFound:
                "That account is no longer available in TokenUsage. Refresh the account list and try again."
            case .targetManagedAccountAuthMissing:
                "TokenUsage could not find saved auth for that account. Re-authenticate it and try again."
            case .targetManagedAccountAuthUnreadable:
                "TokenUsage could not read saved auth for that account. Re-authenticate it and try again."
            case .liveAccountUnreadable:
                "TokenUsage could not read the current system account on this Mac."
            case .liveAccountMissingIdentityForPreservation:
                "TokenUsage could not safely preserve the current system account before switching."
            case .liveAccountAPIKeyOnlyUnsupported:
                "TokenUsage can't replace a system account that is signed in with an API key only setup."
            case .displacedLiveManagedAccountConflict:
                "TokenUsage found another managed account that already uses the current system account. "
                    + "Resolve the duplicate account before switching."
            case .displacedLiveImportFailed:
                "TokenUsage could not save the current system account before switching."
            case .managedStoreCommitFailed:
                "TokenUsage could not update managed account storage."
            case .liveAuthSwapFailed:
                "TokenUsage could not replace the live Codex auth on this Mac."
            }
