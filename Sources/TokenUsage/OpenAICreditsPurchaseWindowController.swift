...
final class OpenAICreditsPurchaseWindowController: NSWindowController, WKNavigationDelegate, WKScriptMessageHandler {
    ...
    private static let logHandlerName = "tokenusageLog"
    private static let debugLogURL = URL(fileURLWithPath: NSTemporaryDirectory())
        .appendingPathComponent("tokenusage-buy-credits.log")
    private static let autoStartScript = """
    (() => {
      if (window.__tokenusageAutoBuyCreditsStarted) return 'already';
      const log = (...args) => {
        try {
          window.webkit?.messageHandlers?.tokenusageLog?.postMessage(args);
        } catch {}
      };
      ...
      const startNextPolling = (initialDelay = 500, interval = 500, maxAttempts = 90) => {
        if (window.__tokenusageNextPolling) return;
        window.__tokenusageNextPolling = true;
      ...
      const observeNextButton = () => {
        if (window.__tokenusageNextObserver || !window.MutationObserver) return;
        const observer = new MutationObserver(() => {
          if (clickNextIfReady(1)) {
            observer.disconnect();
            window.__tokenusageNextObserver = null;
          }
        });
        observer.observe(document.body, { subtree: true, childList: true, attributes: true });
        window.__tokenusageNextObserver = observer;
      };
      ...
      if (findAndClick()) {
        window.__tokenusageAutoBuyCreditsStarted = true;
        startNextPolling();
        observeNextButton();
        logDialogButtons();
        return 'clicked';
    ...
    private let logger = TokenUsageLog.logger(LogCategories.creditsPurchase)
