...
You can also configure the API key in TokenUsage Settings → Providers → OpenRouter.
...
```bash
tokenusage --provider openrouter
tokenusage -p or  # alias
```
...
| `OPENROUTER_X_TITLE` | Optional client title sent as `X-Title` header (defaults to `TokenUsage`) |
