---
summary: "Sparkle integration details for TokenUsage: updater config, keys, and release flow."
read_when:
...
- Updater: `SPUStandardUpdaterController` owned by `AppDelegate` (see `Sources/TokenUsage/CodexbarApp.swift:1`).
...
## Release flow
1) Build & notarize as usual (`./Scripts/sign-and-notarize.sh`), producing notarized `TokenUsage-<ver>.zip`.
...
- TokenUsage disables Sparkle in Homebrew and unsigned builds; those installs should be updated via `brew` or reinstalling from Releases.
