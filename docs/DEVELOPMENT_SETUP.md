...
When developing TokenUsage, you may see frequent keychain permission prompts like:

> **TokenUsage wants to access key "Claude Code-credentials" in your keychain.**

This happens because each rebuild creates a new code signature, and macOS treats it as a "different" app.
That can affect both TokenUsage-owned entries (`com.steipete.TokenUsage`, `com.steipete.tokenusage.cache`) and
...
This creates a self-signed certificate named "TokenUsage Development".

#### 2. Trust the Certificate

1. Open **Keychain Access.app**
2. Find **"TokenUsage Development"** in the **login** keychain
3. Double-click it
...
```bash
export APP_IDENTITY='TokenUsage Development'
```
...
> Note: `compile_and_run.sh` now auto-detects a valid signing identity (Developer ID or TokenUsage Development).
...
If you see multiple `TokenUsage *.app` bundles in your project directory, you can clean them up:

```bash
# Remove all numbered builds
rm -rf "TokenUsage "*.app
...
The build script creates `TokenUsage.app` in the project root. Old numbered builds (like `TokenUsage 2.app`) are created when Finder can't overwrite the running app.
...
This script:
1. Kills existing TokenUsage instances
2. Runs `swift build` (release mode)
3. Runs `swift test` (all tests)
4. Packages the app with `./Scripts/package_app.sh`
5. Launches `TokenUsage.app`
6. Verifies it stays running

When the script falls back to ad-hoc signing, it preserves TokenUsage-owned keychain state by default.
That means you may still see keychain prompts for existing TokenUsage cache entries, but allowing those prompts keeps the
cached browser/OAuth state available across normal rebuilds.
If you want a clean reset of TokenUsage-owned keychain state for an ad-hoc build, run
...
### "TokenUsage is already running"

The compile_and_run script should kill old instances, but if it doesn't:

```bash
pkill -x TokenUsage || pkill -f TokenUsage.app || true
```
