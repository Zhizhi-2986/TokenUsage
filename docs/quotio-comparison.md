...
Quotio is a similar macOS menu bar app for AI quota tracking with **2k stars** and **117 forks**. It has a broader scope (proxy server + quota tracking) but shares many features with TokenUsage.
...
- **TokenUsage**: Direct provider monitoring only (no proxy layer)
...
**Implementation:** Already how TokenUsage works! (We don't have a proxy)
...
**Implementation:** Would require proxy layer (not applicable to TokenUsage's architecture)
**Effort:** Very High (requires proxy server)
**Value:** Low for TokenUsage (out of scope)
...
- **TokenUsage**: Two-bar meter icon, detailed menu card
- **Takeaway**: Both approaches valid; TokenUsage's meter is more info-dense

### Settings Organization
- **Quotio**: Dashboard, Providers, Agents, Quota, Logs, Settings tabs
- **TokenUsage**: Settings → Providers with per-provider toggles
...
- **TokenUsage**: List-based with session/weekly bars
- **Takeaway**: Tier badges are a nice visual touch

---

## Recommended Enhancements for TokenUsage
...
### ❌ Proxy Server Layer
- **Why:** Out of scope for TokenUsage's mission (monitoring, not routing)
