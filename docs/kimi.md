...
Tracks usage for [Kimi For Coding](https://www.kimi.com/code) in TokenUsage.
...
1. Open TokenUsage settings → Providers → Kimi
2. Set "Cookie source" to "Automatic"
3. Enable the Kimi provider toggle
4. TokenUsage will automatically find your session

**Note**: Requires Full Disk Access to read browser cookies (System Settings → Privacy & Security → Full Disk Access → TokenUsage).
...
7. Paste it into the "Auth Token" field in TokenUsage
...
When multiple sources are available, TokenUsage uses this order:
...
- Grant Full Disk Access to TokenUsage in System Settings
...
- **Core files**: `Sources/TokenUsageCore/Providers/Kimi/`
- **UI files**: `Sources/TokenUsage/Providers/Kimi/`
- **Login flow**: `Sources/TokenUsage/KimiLoginRunner.swift`
- **Tests**: `Tests/TokenUsageTests/KimiProviderTests.swift`
