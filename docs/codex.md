...
- Cached cookies: Keychain cache `com.steipete.tokenusage.cache` (account `cookie.codex`, source + timestamp).
...
- RPC reads are bounded: initialization has a longer startup budget, and normal requests have a shorter per-method
  timeout. On timeout, TokenUsage terminates the child `codex app-server` process so the stdout reader unwinds instead
...
- Manual/debug parser only; automatic background refresh and `TokenUsageCLI usage --source cli` do not launch bare Codex TUI.
...
- Cache:
  - Native + merged provider cache: `~/Library/Caches/TokenUsage/cost-usage/codex-v2.json`
...
## Key files
- Web: `Sources/TokenUsageCore/OpenAIWeb/*`
- CLI RPC + diagnostic PTY parser: `Sources/TokenUsageCore/UsageFetcher.swift`,
  `Sources/TokenUsageCore/Providers/Codex/CodexStatusProbe.swift`
