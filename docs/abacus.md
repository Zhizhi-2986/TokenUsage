...
3. Paste the header into the cookie field in TokenUsage
...
```bash
tokenusage usage --provider abacusai --verbose
```
...
Log in to [apps.abacus.ai](https://apps.abacus.ai) in a supported browser (Safari, Chrome, Firefox), then refresh TokenUsage.
