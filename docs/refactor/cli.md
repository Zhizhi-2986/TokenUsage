...
read_when:
  - "Refactoring TokenUsage CLI error handling or config parsing."
...
- (Optional) `tokenusage config dump`
  - Prints normalized config JSON.

## Step-by-step implementation guide
1. **Add validation types**
   - `TokenUsageConfigIssue` + `TokenUsageConfigValidator` in `TokenUsageCore/Config`.
   - Keep file <500 LOC.
...
8. **Verification**
   ...
   - CLI e2e: `tokenusage --json-only ...`, `tokenusage config validate`.
