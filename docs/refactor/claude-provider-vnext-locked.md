...
- CLI token-account behavior must remain consistent in both app and `TokenUsageCLI`.
...
- `.tokenusage` expired credentials: direct refresh endpoint path.
...
- Owner semantics parity remains intact across all sub-phases (`claudeCLI`, `tokenusage`, `environment`).
...
| PR-06 | OAuth decomposition | Extract repository, refresher, and delegated-controller seams from `ClaudeOAuthCredentialsStore` while preserving owner semantics. | R3, R4, R7, R9 | Cache/fingerprint/prompt/owner suites green (`claudeCLI`, `tokenusage`, `environment`). |
...
| R4 | High | Owner semantics drift | Preserve exact owner-to-refresh mapping: `.claudeCLI` delegated, `.tokenusage` direct refresh, `.environment` no refresh. |
