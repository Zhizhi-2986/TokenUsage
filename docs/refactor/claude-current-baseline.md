...
This document is the current-state parity reference for Claude behavior in TokenUsage.
...
This behavior is owned by `Sources/TokenUsageCore/Providers/Claude/ClaudeProviderDescriptor.swift`
...
- Expired credential owner behavior remains owner-specific:
  ...
  - `.tokenusage`: direct refresh path,
...
- CLI-side Claude token-account behavior:
  - OAuth token account changes the effective source mode from `auto` to `oauth`, disables cookie mode, omits a
    manual cookie header, and injects `TOKENUSAGE_CLAUDE_OAUTH_TOKEN`.
...
This behavior is implemented in `Sources/TokenUsageCore/Providers/Claude/ClaudeUsageFetcher.swift`
...
- `Tests/TokenUsageTests/ClaudeBaselineCharacterizationTests.swift`
- `Tests/TokenUsageTests/ClaudeOAuthFetchStrategyAvailabilityTests.swift`
- `Tests/TokenUsageTests/ClaudeUsageTests.swift`
- `Tests/TokenUsageTests/TokenAccountEnvironmentPrecedenceTests.swift`
- `Tests/TokenUsageTests/SettingsStoreCoverageTests.swift`
