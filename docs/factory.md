...
1) **Cached cookie header** (Keychain cache `com.steipete.tokenusage.cache`, account `cookie.factory`).
2) **Stored session** (`~/Library/Application Support/TokenUsage/factory-session.json`).
...
## Session storage
- File: `~/Library/Application Support/TokenUsage/factory-session.json`
...
## Key files
- `Sources/TokenUsageCore/Providers/Factory/FactoryStatusProbe.swift`
- `Sources/TokenUsageCore/Providers/Factory/FactoryLocalStorageImporter.swift`
