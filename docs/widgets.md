---
summary: "WidgetKit snapshot pipeline + visibility troubleshooting for TokenUsage widgets."
read_when:
  - Modifying WidgetKit extension behavior or snapshot format
  - Debugging widget update timing
  - Widget gallery shows no TokenUsage widgets
---
...
## Extension
- `Sources/TokenUsageWidget` contains timeline + views.
- Keep data shape in sync with `WidgetSnapshot` in the main app.

## Widget types
- **TokenUsage Switcher** (`TokenUsageSwitcherWidget`): static provider switcher widget, small/medium/large.
- **TokenUsage Usage** (`TokenUsageUsageWidget`): configurable provider usage widget, small/medium/large.
- **TokenUsage History** (`TokenUsageHistoryWidget`): configurable usage-history chart, medium/large.
- **TokenUsage Metric** (`TokenUsageCompactWidget`): compact credits/today-cost/30-day-cost widget, small only.
...
WAPPEX="$APP/Contents/PlugIns/TokenUsageWidget.appex"
WIDGET_ID="com.steipete.tokenusage.widget" # debug builds use com.steipete.tokenusage.debug.widget
...
```
pluginkit -m -p com.apple.widgetkit-extension -v | grep -i tokenusage || true
...
```
codesign --verify --deep --strict --verbose=4 /Applications/TokenUsage.app
codesign --verify --strict --verbose=4 "$WAPPEX"
codesign --verify --strict --verbose=4 "$WAPPEX/Contents/MacOS/TokenUsageWidget"
spctl --assess --type execute --verbose=4 /Applications/TokenUsage.app
```
...
- Widget bundle id should be `com.steipete.tokenusage.widget` for release and `com.steipete.tokenusage.debug.widget` for debug.
- `NSExtensionPointIdentifier` must be `com.apple.widgetkit-extension`.
- Bundle folder name should match: `TokenUsageWidget.appex`.
