...
> Replicate Codex's direct OAuth token usage in TokenUsage instead of calling the CLI.

## Background

Currently, TokenUsage fetches Codex usage by:
...
- Command: `TokenUsageCLI usage --provider codex --source oauth --json --pretty`
...
|------|----------|---------|
| `CodexOAuthCredentials.swift` | `Sources/TokenUsageCore/Providers/Codex/CodexOAuth/` | Token storage model + loader |
| `CodexOAuthUsageFetcher.swift` | `Sources/TokenUsageCore/Providers/Codex/CodexOAuth/` | API client for usage endpoint |
| `CodexTokenRefresher.swift` | `Sources/TokenUsageCore/Providers/Codex/CodexOAuth/` | Token refresh logic |
...
public enum CodexOAuthUsageFetcher {
    ...
    public static func fetchUsage(
        ...
        request.setValue("TokenUsage", forHTTPHeaderField: "User-Agent")
...
2. Run TokenUsage with debug logging enabled
