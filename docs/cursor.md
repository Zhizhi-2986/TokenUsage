...
1) **Cached cookie header** (preferred)
   ...
   - Keychain cache: `com.steipete.tokenusage.cache` (account `cookie.cursor`).
...
3) **Stored session cookies** (fallback)
   ...
   - Stored at: `~/Library/Application Support/TokenUsage/cursor-session.json`.
...
## Key files
- `Sources/TokenUsageCore/Providers/Cursor/CursorStatusProbe.swift`
- `Sources/TokenUsage/CursorLoginRunner.swift` (login flow)
