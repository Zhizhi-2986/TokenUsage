...
**Fork:** topoffunnel/TokenUsage  
**Upstream 1:** steipete/TokenUsage (original)  
...
```bash
# Your fork (origin)
git remote add origin git@github.com:topoffunnel/TokenUsage.git

# Original upstream (steipete)
git remote add upstream git@github.com:steipete/TokenUsage.git
  ...
  check-upstream:
    ...
    steps:
      ...
      - name: Add upstream remotes
        run: |
          git remote add upstream https://github.com/steipete/TokenUsage.git
      ...
      - name: Create issue if changes detected
        ...
        with:
          script: |
            ...
            **steipete/TokenUsage:** ${upstreamCommits} new commits
            **quotio:** ${quotioCommits} new commits (last week)
            
            Review changes:
            - [steipete commits](https://github.com/steipete/TokenUsage/compare/main...upstream/main)
...
Implemented independently with TokenUsage-specific patterns."
```
...
echo "5. Create PR to steipete/TokenUsage from GitHub UI"
```
...
# Create PR to steipete/TokenUsage via GitHub UI
```
...
Implemented independently using TokenUsage patterns.
```
...
- `Sources/TokenUsage/About.swift` (your attribution)
- `Sources/TokenUsage/PreferencesAboutPane.swift` (fork sections)
...
# Push and create PR
git push origin upstream-pr/fix-cursor-bonus
# Then create PR on GitHub to steipete/TokenUsage
```
...
Implemented independently using TokenUsage's architecture."
```
...
# CONFLICT in Sources/TokenUsage/About.swift

# Keep your fork version for branding files
git checkout --ours Sources/TokenUsage/About.swift
git add Sources/TokenUsage/About.swift
