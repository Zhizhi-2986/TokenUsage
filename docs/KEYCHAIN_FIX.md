...
The original fix (migrating legacy TokenUsage keychain items to `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly`) is
still in place, but the architecture has changed:

- Provider settings and manual secrets are now persisted in `~/.tokenusage/config.json`.
- Legacy keychain stores are still present mainly to migrate old installs, then clear old items.
- Keychain is still used for runtime cache entries (for example `com.steipete.tokenusage.cache`) and Claude OAuth
...
| --- | --- |
| TokenUsage stores provider credentials only in keychain | Manual/provider settings are config-file backed (`~/.tokenusage/config.json`), while keychain is still used for runtime caches and Claude OAuth bootstrap fallback. |
| `ClaudeOAuthCredentials.swift` migrated TokenUsage-owned Claude OAuth keychain items | Claude OAuth primary source is Claude CLI keychain service (`Claude Code-credentials`), with TokenUsage cache in `com.steipete.tokenusage.cache` (`oauth.claude`). |
| Migration runs in `TokenUsageApp.init()` | Migration runs in `HiddenWindowView` `.task` via detached task (`KeychainMigration.migrateIfNeeded()`). |
...
### 1. Legacy TokenUsage keychain migration (V1)
`Sources/TokenUsage/KeychainMigration.swift` migrates legacy `com.steipete.TokenUsage` items (for example
`claude-cookie`) to `AfterFirstUnlockThisDeviceOnly`.

- Gate key: `KeychainMigrationV1Completed`
- Runs once unless flag is reset.
- Covers legacy TokenUsage-managed accounts only (not Claude CLI's own keychain service).

### 2. Claude OAuth bootstrap path
`Sources/TokenUsageCore/Providers/Claude/ClaudeOAuth/ClaudeOAuthCredentials.swift`

Load order for credentials:
1. Environment override (`TOKENUSAGE_CLAUDE_OAUTH_TOKEN`, scopes env key).
2. In-memory cache.
3. TokenUsage keychain cache (`com.steipete.tokenusage.cache`, account `oauth.claude`).
...
When TokenUsage does not have usable OAuth credentials in its own cache (`com.steipete.tokenusage.cache` / `oauth.claude`),
...
`Sources/TokenUsageCore/CookieHeaderCache.swift` and `Sources/TokenUsageCore/KeychainCacheStore.swift`

- Browser-imported Claude session cookies are cached in keychain service `com.steipete.tokenusage.cache`.
...
- Users can clear browser-cookie cache entries from **Preferences → Debug → Caches** or with
  `tokenusage cache clear --cookies`. `--provider <id>` scopes cookie clearing to one provider and includes scoped
...
- Keychain cache store (`com.steipete.tokenusage.cache`) also writes with `ThisDeviceOnly`.
...
```bash
defaults read com.steipete.tokenusage claudeOAuthKeychainDeniedUntil
```

### Inspect keychain-related logs
```bash
log show --predicate 'subsystem == "com.steipete.tokenusage" && (category == "keychain-migration" || category == "keychain-preflight" || category == "keychain-prompt" || category == "keychain-cache" || category == "claude-usage" || category == "cookie-cache")' --last 10m
```
...
- `Sources/TokenUsage/KeychainMigration.swift`
- `Sources/TokenUsage/HiddenWindowView.swift`
- `Sources/TokenUsageCore/Providers/Claude/ClaudeOAuth/ClaudeOAuthCredentials.swift`
- `Sources/TokenUsageCore/Providers/Claude/ClaudeOAuth/ClaudeOAuthKeychainAccessGate.swift`
- `Sources/TokenUsageCore/KeychainAccessPreflight.swift`
- `Sources/TokenUsageCore/KeychainNoUIQuery.swift`
- `Sources/TokenUsageCore/KeychainCacheStore.swift`
- `Sources/TokenUsageCore/CookieHeaderCache.swift`
