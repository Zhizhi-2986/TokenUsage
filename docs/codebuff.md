...
TokenUsage surfaces [Codebuff](https://www.codebuff.com) credit balance and
...
- `GET  https://www.codebuff.com/api/user/subscription` — subscription tier,
  ...
  `weeklyLimit`) when TokenUsage is using the CLI credentials-file session token.

Both endpoints use a Bearer token. Codebuff credentials come from the
environment, the normal TokenUsage config file, or the official CLI credentials
file; the Codebuff provider does not write a separate Keychain credential.

## Authentication

TokenUsage resolves the Codebuff API token in this order:
...
2. The per-provider API key stored in Settings → Providers → Codebuff (saved
   in `~/.tokenusage/config.json` via the normal TokenUsage config flow). API-key
   tokens fetch credit balance only.
3. `~/.config/manicode/credentials.json` — the file the official `codebuff`
   CLI (formerly `manicode`) writes after `codebuff login`. TokenUsage reads
...
- Verify your token works manually:
  ...
  curl -s -X POST -H "Authorization: Bearer $CODEBUFF_API_KEY" \
    -H 'Content-Type: application/json' -d '{"fingerprintId":"tokenusage-usage"}' \
    https://www.codebuff.com/api/v1/usage
  ```

## Related files

- `Sources/TokenUsageCore/Providers/Codebuff/` — descriptor, fetcher, snapshot,
  settings reader, error types.
- `Sources/TokenUsage/Providers/Codebuff/` — settings store bridge + macOS
  settings pane implementation.
- `Tests/TokenUsageTests/CodebuffSettingsReaderTests.swift`,
