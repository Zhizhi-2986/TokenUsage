...
- Newer cache shapes may omit `quotaUsage` but include `usage` counters. In that case TokenUsage derives
...
6. In TokenUsage: Providers → Windsurf → Cookie source → Manual → paste the JSON bundle.
...
- Parsed response fields used by TokenUsage:
...
- Grant Full Disk Access to TokenUsage (System Settings → Privacy & Security → Full Disk Access).
...
## Key files
- `Sources/TokenUsageCore/Providers/Windsurf/WindsurfStatusProbe.swift` (local SQLite)
- `Sources/TokenUsageCore/Providers/Windsurf/WindsurfDevinSessionImporter.swift` (Chromium localStorage extraction)
- `Sources/TokenUsageCore/Providers/Windsurf/WindsurfWebFetcher.swift` (protobuf request + response parsing)
- `Sources/TokenUsageCore/Providers/Windsurf/WindsurfProviderDescriptor.swift` (fetch strategies)
- `Sources/TokenUsage/Providers/Windsurf/WindsurfProviderImplementation.swift` (settings UI)
- `Sources/TokenUsage/Providers/Windsurf/WindsurfSettingsStore.swift` (settings persistence)
