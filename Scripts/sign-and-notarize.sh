...
echo "$APP_STORE_CONNECT_API_KEY_P8" | sed 's/\\n/\n/g' > /tmp/tokenusage-api-key.p8
trap 'rm -f /tmp/tokenusage-api-key.p8 /tmp/${APP_NAME}Notarize.zip' EXIT
...
if [[ -f "$APP_BUNDLE/Contents/Helpers/TokenUsageCLI" ]]; then
  codesign --force --timestamp --options runtime --sign "$APP_IDENTITY" \
    "$APP_BUNDLE/Contents/Helpers/TokenUsageCLI"
fi
if [[ -f "$APP_BUNDLE/Contents/Helpers/TokenUsageClaudeWatchdog" ]]; then
  codesign --force --timestamp --options runtime --sign "$APP_IDENTITY" \
    "$APP_BUNDLE/Contents/Helpers/TokenUsageClaudeWatchdog"
fi
if [[ -d "$APP_BUNDLE/Contents/PlugIns/TokenUsageWidget.appex" ]]; then
  codesign --force --timestamp --options runtime --sign "$APP_IDENTITY" \
    --entitlements "$WIDGET_ENTITLEMENTS" \
    "$APP_BUNDLE/Contents/PlugIns/TokenUsageWidget.appex/Contents/MacOS/TokenUsageWidget"
  codesign --force --timestamp --options runtime --sign "$APP_IDENTITY" \
    --entitlements "$WIDGET_ENTITLEMENTS" \
    "$APP_BUNDLE/Contents/PlugIns/TokenUsageWidget.appex"
fi
codesign --force --timestamp --options runtime --sign "$APP_IDENTITY" \
