...
ZIP=${1:?
"Usage: $0 TokenUsage-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/steipete/TokenUsage/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^TokenUsage-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/steipete/TokenUsage/releases/download/v${VERSION}/"}
...
WORK_DIR=$(mktemp -d /tmp/tokenusage-appcast.XXXXXX)
