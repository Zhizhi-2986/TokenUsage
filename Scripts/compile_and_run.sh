#!/usr/bin/env bash
# Reset TokenUsage: kill running instances, build, package, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${ROOT_DIR}/TokenUsage.app"
APP_PROCESS_PATTERN="TokenUsage.app/Contents/MacOS/TokenUsage"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/.build/debug/TokenUsage"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/.build/release/TokenUsage"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/tokenusage-compile-and-run-${LOCK_KEY}"
...
kill_claude_probes() {
  # TokenUsage spawns `claude /usage` + `/status` in a PTY; if we kill the app mid-probe we can orphan them.
...
kill_all_tokenusage() {
  is_running() {
    pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1 \
      ...
      || pgrep -x "TokenUsage" >/dev/null 2>&1
  }
  ...
  pkill -9 -x "TokenUsage" 2>/dev/null || true
  ...
  fail "Failed to kill all TokenUsage instances."
}
...
# 2) Kill all running TokenUsage instances (debug, release, bundled).
log "==> Killing existing TokenUsage instances"
kill_all_tokenusage
kill_claude_probes
...
if [[ "${SIGNING_MODE:-adhoc}" == "adhoc" && "${CLEAR_ADHOC_KEYCHAIN}" == "1" ]]; then
  log "==> Clearing TokenUsage keychain entries (adhoc signing)"
  # Clear both the legacy keychain store and the current cache service when developers explicitly want a clean reset
  # of TokenUsage-owned keychain state for ad-hoc builds.
  delete_keychain_service_items "com.steipete.TokenUsage"
  delete_keychain_service_items "com.steipete.tokenusage.cache"
elif [[ "${SIGNING_MODE:-adhoc}" == "adhoc" ]]; then
  log "==> Preserving TokenUsage keychain entries (pass --clear-adhoc-keychain to reset adhoc keychain state)"
fi
...
if [[ "${DEBUG_LLDB}" == "1" ]]; then
  run_step "package app" env TOKENUSAGE_ALLOW_LLDB=1 ARCHES="${ARCHES_VALUE}" "${ROOT_DIR}/Scripts/package_app.sh" debug
else
  if [[ -n "${SIGNING_MODE}" ]]; then
    run_step "package app" env TOKENUSAGE_SIGNING="${SIGNING_MODE}" ARCHES="${ARCHES_VALUE}" "${ROOT_DIR}/Scripts/package_app.sh"
  else
...
if ! open "${APP_BUNDLE}"; then
  ...
  "${APP_BUNDLE}/Contents/MacOS/TokenUsage" >/dev/null 2>&1 &
  disown
fi
