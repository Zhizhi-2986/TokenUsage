...
# Check if we already have a TokenUsage development certificate
CERT_NAME="TokenUsage Development"
...
openssl req -x509 -newkey rsa:4096 -sha256 -days 3650 \
    -nodes -keyout /tmp/tokenusage-dev.key -out /tmp/tokenusage-dev.crt \
    -config "$TEMP_CONFIG" 2>/dev/null

# Convert to PKCS12 format
openssl pkcs12 -export -out /tmp/tokenusage-dev.p12 \
    -inkey /tmp/tokenusage-dev.key -in /tmp/tokenusage-dev.crt \
    -passout pass: 2>/dev/null

# Import into keychain
security import /tmp/tokenusage-dev.p12 -k ~/Library/Keychains/login.keychain-db -T /usr/bin/codesign -T /usr/bin/security

# Clean up temporary files
rm -f /tmp/tokenusage-dev.{key,crt,p12}
